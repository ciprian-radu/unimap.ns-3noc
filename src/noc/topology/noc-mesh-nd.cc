...
NS_LOG_COMPONENT_DEFINE ("NocMeshND");

namespace ns3
{

  NS_OBJECT_ENSURE_REGISTERED ( NocMeshND);

  TypeId
  NocMeshND::GetTypeId ()
  {
    static TypeId tid = TypeId ("ns3::NocMeshND")
        .SetParent<NocTopology> ()
        .AddConstructor<NocMeshND> ()
        .AddAttribute ("Size",
              "how many nodes the nD mesh will have on each line",
              ObjectVectorValue(),
              MakeObjectVectorAccessor (&NocMeshND::m_size),
              MakeObjectVectorChecker<NocValue> ())
     ;
    return tid;
  }

  NocMeshND::NocMeshND () : NocTopology ()
  {
    NS_LOG_FUNCTION_NOARGS ();
  }

  NocMeshND::NocMeshND (vector<Ptr<NocValue> > size) : NocTopology ()
  {
    NS_LOG_FUNCTION_NOARGS ();
    m_size = size;
  }
  ...
  uint32_t
  NocMeshND::GetNumberOfNodes (int32_t dimension)
  {
  ...
  NetDeviceContainer
  NocMeshND::Install (NodeContainer nodes)
  {
    NS_LOG_FUNCTION_NOARGS ();
    ...
    NS_LOG_DEBUG ("Printing the "<<m_size.size() <<"D mesh topology (channels <-> net devices <-> nodes)...");
    for (uint32_t i = 0; i < m_devices.GetN (); ++i)
      ...
      } NS_LOG_DEBUG ("Done with printing the "<<m_size.size() <<"D mesh topology.");

    return m_devices;
  }

  vector<uint8_t>
  NocMeshND::GetDestinationRelativeDimensionalPosition (uint32_t sourceNodeId, uint32_t destinationNodeId)
  {
    ...
    for (unsigned int k = 0; k < m_size.size(); k++)
      {
        if (destination.at (k) < source.at (k))
          {
            relative.at(k) = NocHeader::DIRECTION_BIT_MASK;
          }
        relative.at (k) = relative.at (k) | std::abs ((int) (destination.at (k) - source.at (k)));
        if ((relative.at (k) & NocHeader::DIRECTION_BIT_MASK) == NocHeader::DIRECTION_BIT_MASK)
          {
            NS_LOG_DEBUG ("dimension " << (k+1) << " relative " << " -" << (int) (relative.at(k) & NocHeader::OFFSET_BIT_MASK));
          }
        else
          {
            NS_LOG_DEBUG ("dimension " << (k+1) << " relative " << (int) (relative.at(k) & NocHeader::OFFSET_BIT_MASK));
  ...
  void
  NocMeshND::SaveTopology (NodeContainer nodes, string directoryPath)
  {
        ...
        try
          {
            ...
            nodeType theNode (id);
            nodeType::topologyParameter_type::topology_type topology ("meshND");
